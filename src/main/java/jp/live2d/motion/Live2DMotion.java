...
public class Live2DMotion extends AMotion {
        ...
        while (i < bufSize) {
            ...
            if (!(c == 10 || c == 13)) {
                if (c == '#') {
                    ...
                    if (eqpos >= 0) {
                        if (eqpos == head + 4 && str[head + 1] == 102 && str[head + 2] == 112 && str[head + 3] == 115) {
                            is_fps = true;
                        }
                        i = eqpos + 1;
                        while (i < bufSize) {
    ...
    public void dump() {
        for (int i = 0; i < this.motions.size(); i++) {
            ...
            System.out.printf("paramID[%s] [%d]. ", new Object[]{m.paramIDStr, Integer.valueOf(m.values.length)});
            int j = 0;
            while (j < m.values.length && j < 10) {
                System.out.printf("%5.2f ,", new Object[]{Float.valueOf(m.values[j])});
                j++;
            }
            System.out.printf("\n", new Object[0]);
        }
    ...
    public void updateParamExe(ALive2DModel model, long timeMSec, float weight, MotionQueueManager.MotionQueueEnt motionQueueEnt) {
        ...
        for (int i3 = 0; i3 < this.motions.size(); i3++) {
            ...
            } else if (100 > m.motionType || m.motionType > 105) {
