...
class KanojoAdapter(context: Context?, private val mDic: DynamicImageCache) : BaseKanojoPairAdapter(context), ObservableAdapter {
	...
	private val mRunnableLoadPhotos: Runnable = object : Runnable {
		override fun run() {
			if (mLoadedPhotoIndex < this@KanojoAdapter.count) {
				...
				val kanojoAdapter2 = this@KanojoAdapter
				val `access$0` = kanojoAdapter2.mLoadedPhotoIndex
				kanojoAdapter2.mLoadedPhotoIndex = `access$0` + 1
				val a = kanojoAdapter.getItem(`access$0`) as ActivityModel
	...
	override fun addModelList(l: ModelList<KanojoPair>) {
		super.addModelList(l)
		for (pair in l) {
			val kanojo = pair.left
			if (kanojo != null) {
				mDic.requestBitmap(kanojo.profile_image_url)
			}
			val kanojo2 = pair.right
			if (kanojo2 != null) {
				mDic.requestBitmap(kanojo2.profile_image_url)
			}
		}
	}
